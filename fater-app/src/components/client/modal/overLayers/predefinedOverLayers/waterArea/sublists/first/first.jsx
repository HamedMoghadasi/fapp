    ...
    tabs: [
      {
        ...
        handleAddLayer: (selectedSatellites, location) => {
          ...
          selectedSatellites.map((satellite) => {
            ...
            let heatmapUrl = getHeatMapUrl(
              { start: "1593568800", end: "1593568800" },
              {
            ...
            let heatmapChangeUrl = getHeatMapChangeUrl(
              { start: "1593568800", end: "1593568800" },
              {
                parameter: "cstlnchange",
                location: location,
            ...
            console.log("heatmapUrl :>> ", heatmapUrl);
            console.log("heatmapChangeUrl :>> ", heatmapChangeUrl);
            if (heatmapUrl && heatmapChangeUrl) {
              const mapContainer = $("#mapContainer").data("map");
              ...
              raster_change.on("beforeoperations", function (event) {
                event.data.colors = getColors__change();
              });

              raster_change.on("afteroperations", function (event) {});

              function getColors__change() {
                var scale = chroma
                  .scale(["#b21227", "#fec97c", "#dff1e3", "#353f9a"])
                  .colors(254);
              ...
              let tilelayer_change = new TileLayer({
                source: aerial_change,
              });

              let heatmap_change = new ImageLayer({
                source: raster_change,
              });

              const heatmapName_change =
                satellite === "default"
                  ? "Coastline changes (Old)"
                  : `Coastline changes (Old) -- ${getSatelliteLableByValue[satellite]}`;
              heatmap_change.set("name", heatmapName_change);
              heatmap_change.set("description", `location: ${location} `);
              heatmap_change.set("colors", [
                "#b21227",
                "#fec97c",
                "#dff1e3",
                "#353f9a",
              ]);
              heatmap_change.set("params", [
                "cstlnchange",
                `${location}`,
                `${satellite}`,
              ]);
              heatmap_change.set("isHeatMap", true);
              const zIndex_change =
                mapContainer.getLayers().array_.length * 10000;
              heatmap_change.setZIndex(zIndex_change);

              //mapContainer.getLayers().array_.push(tilelayer);
              mapContainer.getLayers().array_.push(heatmap_change);

              this.props.refreshComponent();
              var i_change = 0;
              var timer_change = setInterval(function () {
                if (i_change === 6) clearInterval(timer_change);
                mapContainer.updateSize();
                i++;
              }, 500);
              var aerial = new XYZ({
              ...
              toast.error("در این بازه داده ای برای نمایش وجود ندارد", {
                autoClose: 10000,
                fontSize: "20px",
                position: toast.POSITION.TOP_RIGHT,
              });
